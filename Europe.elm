...
import GameMapData exposing (GameMapData, EmpireID, Piece, MapData, LocationID(..), ProvinceData(..), TerrainType(..))
...
nothingData =
    ...
    , Capital ("Portugal",
        ...
        { polygon = "32,396 30,406 17,427 14,427 10,433 13,440 15,441 12,450 13,454 8,462 19,469 27,468 36,457 34,447 40,441 37,431 42,432 52,412 61,411 62,407 55,400 42,399 43,395 32,396"
        , coordinates = (34,417)
        , terrainType = L }
        )
    , Capital ("Denmark",
    ...
    , Capital ("Spain",
        ...
        { polygon = "134,417 123,412 113,407 112,399 101,396 96,397 72,384 59,381 54,375 48,374 46,378 39,375 33,381 35,384 32,396 43,395 42,399 55,400 62,407 61,411 52,412 42,432 37,431 40,441 34,447 36,457 27,468 33,475 34,484 37,490 47,488 52,489 60,486 78,491 83,494 86,485 90,483 98,484 107,474 113,473 115,469 110,461 124,444 131,439 146,438 157,432 158,425 154,427 142,417 135,414 134,417 123,412 134,417"
        , coordinates = (64,439)
        , terrainType = L }
        )
    , Capital ("Tunis",
    ...
    , Capital ("Bulgaria",
        ...
        { polygon = "371,438 366,439 371,456 365,461 369,464 376,464 388,460 392,472 400,468 408,470 413,464 412,454 420,451 426,450 422,441 425,427 429,426 430,423 422,420 410,420 404,422 398,427 390,425 382,427 375,423 370,425 367,421 365,425 368,433 371,438"
        , coordinates = (395,443)
        , terrainType = L }
        )
    , Capital ("Rumania",
        ...
        { polygon = "403,360 404,371 394,376 395,382 401,385 406,396 401,402 387,402 367,406 365,412 367,421 370,425 375,423 382,427 390,425 398,427 404,422 410,420 422,420 430,423 432,409 439,404 438,397 427,399 422,382 423,376 414,372 411,361 403,360"
        , coordinates = (415,405)
        , terrainType = L }
        )
    , Capital ("Serbia",
...
seas = List.map Noncapital
    ...
    , ("North Sea",
        ...
        { polygon = "171,197 171,181 198,154 241,154 241,224 248,224 245,237 211,237 211,301 173,301 165,293 140,197 171,197"
        , coordinates = (204,215)
        , terrainType = W }
        )
    , ("Skaggerrack",
